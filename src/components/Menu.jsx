...
const Menu = () => {
  ...
  return (
    <section id='menu' aria-labelledby='menu-heading'>
      ...
      <h2 id='menu-heading' className='sr-only'>Cokctail Me</h2>
      <nav className='cocktail-tabs' aria-label="Cocktail Navigation">
        {sliderLists.map((cocktail,index)=>{
          const isActive = index === currentIndex;

          return (
            <button onClick={()=>goToSlide(index)} key={cocktail.id} className={`${isActive ? "text-amber-500 border-amber-500":"text-white/50 border-white/50"}`}>{cocktail.name}</button>
          )
        })}
      </nav>
      <div className='content'>
        ...
        <div className='recipe'>
          ...
          <div className='details'>
            <h2 className='text-amber-500'>{currentCocktail.title}</h2>
